...
void update_helper(muse::Animation *animation, std::vector<glm::mat4> &transforms,
                   const glm::mat4 &root_inverse_transform, muse::Bone *bone, glm::mat4 &parent_transform)
{
...
void find_bone_helper(std::string_view name, muse::Bone &bone, muse::Bone *out_bone)
{
    if (bone.name == name)
    {
...
Skeleton::Skeleton(const Bone &root_bone, Animation *animation)
    : root_bone_(root_bone)
    , transforms_(100)
